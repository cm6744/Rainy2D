...
public class Tokenizer extends Lookahead<Token> {
    ...
    protected Token endOfInput() {
        return Token.createAndFill(TokenType.EOI, input.current());
    }
    ...
    protected Token fetch() {
        ...
        if (isAtBracket(false)) {
            return Token.createAndFill(TokenType.SYMBOL, input.consume());
        }
    ...
    protected Token fetchString() {
        ...
        Token result = Token.create(TokenType.STRING, input.current());
    ...
    protected Token handleKeywords(Token idToken) {
        ...
        if (keyword != null) {
            Token keywordToken = Token.create(TokenType.KEYWORD, idToken);
    ...
    protected Token fetchSpecialId() {
        Token result = Token.create(TokenType.SPECIAL_ID, input.current());
        result.addToTrigger(input.consume());
        while (isIdentifierChar(input.current())) {
            result.addToContent(input.consume());
        }
    ...
    protected Token fetchSymbol() {
        Token result = Token.create(TokenType.SYMBOL, input.current());
        result.addToTrigger(input.consume());
    ...
    protected Token fetchNumber() {
        Token result = Token.create(TokenType.INTEGER, input.current());
        result.addToContent(input.consume());
    ...
    public void consumeExpectedSymbol(String symbol) {
        if (current().matches(TokenType.SYMBOL, symbol)) {
            consume();
        } else {
    ...
    public void consumeExpectedKeyword(String keyword) {
        if (current().matches(TokenType.KEYWORD, keyword)) {
            consume();
        } else {
